...
namespace Pos.WebApi.Features.Sales.Entities
{
    public class OrderSaleDetail
    {
        ...
        public class Map
        {
            public Map(EntityTypeBuilder<OrderSaleDetail> builder)
            {
                ...
                builder.HasOne(x => x.OrderSale).WithMany(x => x.Detail).HasForeignKey(x => x.DocId);
                builder.ToTable("OrderSaleDetail");
            }
        }
    }
}
