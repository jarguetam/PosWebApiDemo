...
{
    public class PaymentPurchaseDetail
    {
        ...
        [JsonIgnore]
        public PaymentPurchase PaymentPurchase { get; set; }
        public bool IsValid()
        {
        ...
        public class Map
        {
            public Map(EntityTypeBuilder<PaymentPurchaseDetail> builder)
            {
                ...
                builder.HasOne(x => x.PaymentPurchase).WithMany(x => x.Detail).HasForeignKey(x => x.DocId);
                builder.ToTable("PaymentPurchaseDetail");
            }
        }
    }
}
